...
var setIn__default = /*#__PURE__*/_interopDefaultLegacy(setIn);
...
const importUpload = async (request) => {
  ...
  if (contentType.includes('multipart/form-data')) {
    ...
    return await importer.importDirectory(files)
  } else {
    const content = await request.arrayBuffer();
    return await importer.importBlob(new Uint8Array(content))
  }
};
...
const importAsset = async (file) => {
  const { cid } = await importer.importDirectory([file]);
  return multiformats.CID.parse(cid.toString())
};
  ...
  if (contentType.includes('multipart/form-data')) {
    ...
    for (const [name, content] of form.entries()) {
      if (name !== 'meta') {
        ...
        setIn__default['default'](data, path, href);
        setIn__default['default'](dag, path, cid);
      }
    }

    const metadata = await importer.importBlob(JSON.stringify(data));

    const bytes = CBOR__namespace.encode({
      ...dag,
      'metadata.json': metadata.cid,
      type: 'nft',
    });
    const hash = await sha2.sha256.digest(bytes);
    const ipnft = multiformats.CID.create(1, CBOR__namespace.code, hash);
 ...
 * @param {State} state
 */
const handle = async (request, { store, AUTH_TOKEN }) => {
  ...
  if (request.method === 'OPTIONS') {
    return new mockServer.Response('', { headers: headers(request) })
  }
  ...
  try {
    switch (`${request.method} /${pathParts.join('/')}`) {
      case 'POST /upload': {
        ...
        return new mockServer.Response(JSON.stringify(result), {
      ...
      case `GET /check/${pathParts[1]}`: {
        const cid = multiformats.CID.parse(pathParts[1] || '');
      ...
      case `GET /${pathParts[0]}`: {
        authorize();
        const cid = multiformats.CID.parse(pathParts[0] || '');
      ...
      case `DELETE /${pathParts[0]}`: {
        ...
        return new mockServer.Response(JSON.stringify({ ok: true }), {
          headers: headers(request),
        })
      }
      default: {
